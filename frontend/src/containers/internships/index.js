import React, {useEffect} from 'react';
import {  Link } from "react-router-dom";
import { getInternships} from '../../store/actions/mainActions'
import {withRouter} from 'react-router-dom'
import { connect } from 'react-redux'
import Header from '../../components/header'
import Footer  from '../../components/footer'
import './internships.css'
const onMount = props => () => {
    props.getInternships()
   
  }

function Iternship(props) {

    useEffect(onMount(props), [])

    const {internships} = props.mainReducer
    const listi = internships.map(item => (
      <div className='boxs col-5 col-s-5'> 
       <h5 >{item.title}</h5>
       <hr />
    <p className='des' >{item.description}</p>
    <p >Компания : {item.company.name}</p><p >Сфера : {item.job_area.title}</p>
    <p className='des' > Продолжительность:  {item.duration} мес.</p>
    <p className='des' > Минимальная зарплата:  {item.salary} тг.</p>
    <p >Начало: {item.start_date}</p>
    <div className='row'>{item.techno.map(i => ( <p className='techno'>{i.title} </p>))}</div>
       </div>
      
    ))
  

  return (
    <div >
       <Header />
       <div className='container don'>
           <div className='row'>
           {listi}
           </div>
       </div>
       <Footer />
    </div>
  );
}


const mapStateToProps = state =>({
    mainReducer: state.mainReducer
  })
  
  const mapDispatchToProps = {
     getInternships
  }
  
  export default connect(
    mapStateToProps,
    mapDispatchToProps
  )(withRouter(Iternship))